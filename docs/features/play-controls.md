...
On MiSTer, game tracking requires `recents=1` and `log_file_entry=1` in `MiSTer.ini`. Without them, games launched from the MiSTer menu are not detected. See [MiSTer game tracking](../platforms/mister/index.md#game-tracking).
