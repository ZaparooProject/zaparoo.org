...
description: "Zaparoo Core v2.12.0 adds Launcher backend support, media browsing and metadata improvements, gamelist.xml scraping, favorites, MiSTer launcher updates, and reliability fixes."
tags: [software, core]
authors: [wizzo]
---

Zaparoo Core v2.12.0 is now available. This release adds the Core-side support needed for the new [Zaparoo Launcher](/docs/launcher/), including faster media browsing, richer metadata and artwork APIs, per-system launcher settings, and favorites.
...
- **[Zaparoo Launcher support](#zaparoo-launcher-support)**: faster media browsing, favorites, metadata, artwork, and launcher settings for the TV UI
...
## Zaparoo Launcher support

The first [Zaparoo Launcher](/docs/launcher/) release used a development Core build. Core v2.12.0 brings that backend work into a normal Core release.
...
On MiSTer, this is mainly useful for getting artwork into [Zaparoo Launcher](/docs/launcher/#scraping-metadata). If you use Skraper, set it to RecalBox mode so it writes the `gamelist.xml` files Core expects.
