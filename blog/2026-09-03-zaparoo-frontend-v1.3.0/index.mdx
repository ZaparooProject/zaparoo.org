...
Frontend is a lot faster and has a new look, plus a Hub you can arrange yourself, a reorganized settings menu, and a MiSTer Main build that adds kiosk mode and CRT settings to the OSD. Core v2.17.1 is a patch release for problems found after v2.17.0. The two that mattered most: RetroDECK and EmuDeck games refusing to launch on Linux and SteamOS, and MiSTer arcade games counted as closed the moment they started.
...
- The top status strip shows the page number in grid layout and your position in list layout.
...
![Game details dialog with box art and a metadata table](./game_details.png)
...
![Game options menu with Details first, then Add to favorites, Change launcher, Write with App, Add to Hub, and Update metadata](./game_options_menu.png)
...
- [`launch.title`](/docs/zapscript/launch/#launchtitle) and `@System/Title` could boot the first `.bin` track of a disc folder instead of its `.cue` or playlist. Wrong entries already cached are cleared on upgrade.
...
- A per-reader [`scan_mode`](/docs/core/config/readers/#scan-mode) was ignored when the reader was configured with an older driver ID such as `pn532_uart`.
...
- A page of token history could take over a minute to load. ZapScript is now capped at 8192 bytes on every reader, the `run` method, Zap Links, and mapping overrides, so parsing stays bounded. For comparison, an NTAG216 holds 888 bytes.
- With encryption set to optional, scrape progress could stop updating mid-scrape. Notifications sent before a client's first message went out unencrypted and knocked that client out of sync.
...
- USB reader IDs were built from an empty port path on Linux, so they change once after this update. They are runtime identifiers and nothing in your config refers to them.
