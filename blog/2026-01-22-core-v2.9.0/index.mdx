...
description: "Zaparoo Core v2.9.0: major TUI overhaul with themes, Kodi launchers, LaunchBox improvements, Batocera auto-ROM discovery, and PN532 driver update."
tags: [software, core]
authors: [wizzo]
draft: false
---
...
The terminal UI has been completely overhauled with a focus on usability and better integration with the API settings system. TUI-specific options are now stored in a separate [tui.toml](/docs/core/tui/#tui-settings) file.
...
This sets `server_url` for all Kodi launchers without needing to configure each one individually. The `launcher` field in [`[launchers.default]`](/docs/core/config/launchers/#launchersdefault) matches both launcher IDs and group names.

## Custom Launcher Enhancements

[Custom launchers](/docs/features/custom-launchers/) can now use features previously only available to official launchers:
...
Optional [API key authentication](/docs/core/config/service/#api-keys) is now available to protect remote API access. Localhost connections remain allowed without authentication.
