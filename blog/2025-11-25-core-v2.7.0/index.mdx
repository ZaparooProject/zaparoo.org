...
**Readers:** [RS232 barcode scanners](/docs/readers/barcode/rs232/) are now supported, and if the new NFC driver isn't working for you, there's a [legacy driver option](/docs/readers/drivers/#legacy-nfc-drivers) using `legacypn532uart` and `legacypn532i2c`.
...
**Config & integration:** There's a new notification publisher for pushing API notifications to external services. [API security](/docs/core/config/#api_listen) got `api_listen` for binding to specific interfaces and `allowed_ips` for IP allowlists. You can set [custom audio files](/docs/core/config/#success_sound) for success/fail sounds. And [`.zaparooignore`](/docs/core/config/#index_root) files let you exclude directories from media scanning.
