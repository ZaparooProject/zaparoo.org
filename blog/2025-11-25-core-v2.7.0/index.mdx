...
description: "Zaparoo Core v2.7.0: title IDs as default, new tags system, overhauled media database, playtime limits, full Linux desktop support, and more."
tags: [software, core]
authors: [wizzo]
---
...
## [Tags System](/docs/features/tags/)

Tags are the other half of the new game matching system. As media is indexed, Zaparoo now extracts and assigns [tags](/docs/features/tags/) from filenames (and eventually scraped metadata). You can use these tags to filter searches and resolve conflicts.
...
MiSTer gets a lot of love this release. The big ones: **[video playback](/docs/platforms/mister/launchers/#video-playback)** has been completely overhauled using nextfvp ([download it here](https://zaparoo.org/files/fvp-mister_arm-15ea265.zip)), and **[ScummVM](/docs/platforms/mister/launchers/#scummvm)** is now fully supported via the [bbond007 script](https://github.com/bbond007/MiSTer_ScummVM). Neo Geo Pocket/Color are now working, Atari 5200 got some missing file extensions, and you can view and download logs directly from the TUI.
...
The [LaunchBox integration](/docs/platforms/windows/launchers/#launchbox) got a complete rewrite with a new plugin system for more reliable launching and active game tracking. RetroBat now works correctly, and Steam properly detects non-Steam games and shortcuts.

### [Batocera](/docs/platforms/batocera/)

Installation is now much easier with a [new pacman package](/docs/platforms/batocera/#install) and a "Ports" script to open the TUI directly from EmulationStation. There's also background active media detection, improved Kodi launching, and corrected file extensions for 22 systems. We're working with the Batocera team to get Zaparoo officially included in the Content Downloader.

## [Playtime Limits & Parental Controls](/docs/features/play-controls/#playtime-limits)

A new [time management system](/docs/features/play-controls/#playtime-limits) lets you set daily playtime limits, track gaming sessions, and automatically enforce limits with grace period notifications. Perfect for parents who want to manage screen time.
...
**Readers:** [RS232 barcode scanners](/docs/readers/barcode/#hardware-scanners) are now supported, and if the new NFC driver isn't working for you, there's a [legacy driver option](/docs/readers/drivers/#legacy-nfc-drivers) using `legacypn532uart` and `legacypn532i2c`.
...
**Config & integration:** There's a new notification publisher for pushing API notifications to external services. [API security](/docs/core/config/service/#api_listen) got `api_listen` for binding to specific interfaces and `allowed_ips` for IP allowlists. You can set [custom audio files](/docs/core/config/media/#success_sound) for success/fail sounds. And [`.zaparooignore`](/docs/core/config/launchers/#index_root) files let you exclude directories from media scanning.

**ZapScript:** [`launch.random`](/docs/zapscript/launch/#launchrandom) is way faster now, especially for huge libraries. [`launch.title`](/docs/zapscript/launch/#launchtitle) is the new explicit command for title ID launching. The [`?system=<id>`](/docs/zapscript/launch/) argument works for local file paths. And [`**playlist.open`](/docs/zapscript/playlist/#playlistopen) with no arguments reloads the current playlist.
