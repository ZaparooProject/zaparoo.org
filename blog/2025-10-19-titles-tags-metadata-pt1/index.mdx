...
description: "A deep dive into how Zaparoo matches NFC cards to game files: the game identification problem, why it's hard, and the iterations taken to solve it. Part 1."
tags: [core]
authors: [wizzo]
---
...
So, in the previous versions, you are probably aware when you write a game to an [NFC card](/docs/tokens/nfc), it will look something like this: `PSX/Some/Folder/The Game.iso`
...
So that finally brings us to about a month ago. Lately I've been asked by a few third party developers if the [Zaparoo API](/docs/core/api) could be used as a back-end for their own launcher UIs. Totally, I always wanted that to happen, but that also relies on game matching working! So I figured OK let's really knuckle down and finally solve this for real.
...
Game filenames are all over the place, even for curated packs like [No-Intro](https://no-intro.org/) and [TOSEC](https://www.tosec.org/). They have conflicting standards, they have mistakes, and all bets are off when the game is from some random site on its own.
...
- Following from that, IDs can be generated independently. Some random third party website can using results from a source like [IGDB](https://www.igdb.com/) to create a "Zaparoo card ID" with only the most basic understanding of the format, and no need to directly connect to a device.
...
**Update:** [Part 2 is now available!](/blog/titles-tags-metadata-pt2) Read on to learn about the tags system, handling title conflicts, and the complete ID resolution process.
