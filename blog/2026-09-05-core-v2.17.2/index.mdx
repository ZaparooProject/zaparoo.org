...
- **Turning on [`auto_detect`](/docs/core/config/readers/#readers-auto-detect) no longer needs a restart.** It applies within seconds.
...
- **Descriptions follow your language.** The scraper picks synopses by [`media.default_langs`](/docs/core/config/media/#default_langs) instead of English only, gives games listed in `gameinfo.tsv` without an image their year, genre, developer, and players anyway, and reports progress during a step. Run Update metadata with the `mister-docs` source again after updating.
...
- **`pause_on_launch` survives API edits.** An API client updating system defaults dropped [`pause_on_launch`](/docs/core/config/launchers/#pause_on_launch) from `config.toml`, because [`settings.update`](/docs/core/api/methods/#settingsupdate) could not carry it.
