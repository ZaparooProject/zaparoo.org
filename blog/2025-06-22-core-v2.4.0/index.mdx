...
Zaparoo Core v2.4.0 is now available! Lots of new features here plus support for a new platform. A special thank you to [BossRighteous](https://github.com/BossRighteous) and [Anime0t4ku](https://github.com/Anime0t4ku) for the contributions to this release!
...
This is a very high-level overview of what could be possible, but I hope to have some real stuff to show everyone soon!
...
This is *actually* support for [Kodi](https://kodi.tv/), and I plan to port this support over to the Batocera platform, which also ships with Kodi, once it's more stable. It will probably also work on any other OS that is mainly Linux + Kodi.

## Terminal UI (TUI)

The TUI has had a major overhaul which fixes many bugs and achieves two things:
...
This also marks a big step in almost fully replacing the old TapTUI script which used to ship on MiSTer. This script was very popular (and for some more accessible), and it's taken a very long time for us to reach feature parity. This new release covers all major essential features with only some higher-complexity ones like Amiiboo database support and a command builder.
...
- Some minor updates to the layout of the systray menu.

## Custom Launchers

To also help improve platforms like Windows, I've introduced a new feature called Custom Launchers. These are special little .toml files (similar to the config.toml) which tell Core how it should handle a custom path to a file, so you can write your own launchers.

This effectively replaces the use of .bat files for launching on Windows, so you don't need to do things like create a new batch file for every single game. It will be much closer to the experience with natively supported launchers.

## Other Features

- Web UI had been updated to 1.7.0.
- Added a new "on_scan" hook to run custom ZapScript every token scan.
- Added a new "on_remove" hook to run custom ZapScript on token removal with hold mode active.
