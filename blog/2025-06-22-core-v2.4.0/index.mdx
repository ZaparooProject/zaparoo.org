...
description: "Zaparoo Core v2.4.0: new platform support and features with community contributions from BossRighteous and Anime0t4ku."
tags: [software, core]
authors: [wizzo]
---
...
- [on_scan](/docs/core/config/readers/#on_scan): run a custom snippet of ZapScript every time a token is scanned.
- [on_remove](/docs/core/config/readers/#on_remove): run a custom snippet of ZapScript when a token is removed from the reader, if hold mode is enabled.

These work similarly to the existing [before_exit](/docs/core/config/launchers/#before_exit) option but give some greater flexibility for your setup. They may also be useful for platforms which don't have "active media" (now playing) tracking yet.
