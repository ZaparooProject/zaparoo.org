...
description: "Zaparoo Core v2.8.0: hold mode fix, Bazzite and ChimeraOS beta, EmuDeck/RetroDECK launchers, mDNS discovery, blocking hooks, and Steam integration."
tags: [software, core]
authors: [wizzo]
draft: false
---
...
Zaparoo now [advertises itself](/docs/core/config/service/#servicediscovery) on your local network using mDNS. Mobile apps and other clients can automatically find and connect to your Zaparoo instance without needing to enter IP addresses manually. Access Zaparoo using friendly `.local` addresses like `http://mister.local:7497`.
...
Three hooks can now block actions by returning an error: [`on_scan`](/docs/core/config/readers/#on_scan), [`on_remove`](/docs/core/config/readers/#on_remove), and the new [`before_media_start`](/docs/core/config/launchers/#before_media_start). Scripts executed via `**execute:` now receive a `ZAPAROO_ENVIRONMENT` JSON variable with full system state.
...
## [Zap Link](/docs/zapscript/zap-links/) Improvements
...
- [Zap Link](/docs/zapscript/zap-links/) hosts that don't support the protocol are automatically re-checked after 30 days.
