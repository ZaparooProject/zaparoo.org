...
Background playlists can repeat one track or the whole playlist. By default, background music pauses when a game starts and resumes when the game quits, so it does not compete with game audio. You can change that per system in [Core config](/docs/core/config/media/#audio).
...
Core can also back up the whole user database now, which was not possible before. As well as your favorites and launcher overrides, that covers your scan and play history and your token mappings. Core keeps a rolling set of automatic backups, you can take a manual backup that is kept until you remove it, and you can restore from any backup, so you can keep copies of data you care about. Core takes a safety backup before each restore, and can recover on its own if the database is found to be corrupt on startup. You can create, list, and restore backups from the [command line](/docs/core/cli/#back-up-and-restore-device-data) or the API.
