...
<WarpCallout platform="mister" utmContent="backup_post" />
...
Core's cloud-backup endpoint can also be changed in configuration without forking anything. There isn't a supported drop-in server for it today, and I haven't published the server API yet because the protocol is still new and I need room to make breaking changes while it settles. Once it does, I will publish a stable, documented API on Zaparoo.org. This is the same approach I took with [Zap Links](/docs/zapscript/zap-links/), where the protocol and a working self-hosted example are already documented.
