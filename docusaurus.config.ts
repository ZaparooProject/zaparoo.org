  ...
  plugins: [
    ...
    [
      "docusaurus-plugin-remote-content",
      {
        ...
        modifyContent(filename: string, content: string) {
          // Upstream links the config reference as one page; here it is a
          // directory, so point plain links at its index page.
          let modified = content.replace(
            /\]\(\.\.\/\.\.\/core\/config\.md\)/g,
            "](../config/index.md)",
          );
          if (filename === "methods.md") {
            modified = modified.replace(
              /\]\(\.\/encryption\/?\)/g,
              "](./encryption.md)",
            );
