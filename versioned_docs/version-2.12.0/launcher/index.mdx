...
The release linked on this page is not the newest Launcher build. A new stable release is still a couple of weeks away, but development builds shared in Discord already include significant improvements. Join the [Zaparoo Discord](https://zaparoo.org/discord) and check the `#development` channel before installing from the links on this page.
...
Launcher requires Zaparoo Core v2.12.0 or newer. Early Launcher release ZIPs may still include their own compatible Core build.

Launcher currently comes as a manually installed set of files:
