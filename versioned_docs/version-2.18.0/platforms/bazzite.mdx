...
:::warning[Beta]
Bazzite is a [beta platform](./index.mdx#support-levels): Core installs and runs, but some launchers and setup paths are still being completed.
:::
...
Encryption is off by default on Bazzite, so the Zaparoo App and browsers connect without pairing. To require pairing with a PIN, set [`encryption = true`](../core/config/service.md#encryption) in `config.toml`; Core then shows the PIN under **Settings > Clients > Pair** in the terminal UI or with `~/.local/bin/zaparoo -pair`.
