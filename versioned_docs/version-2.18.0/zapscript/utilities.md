...
Stop only [background audio](../features/audio.md) and close any background playlist, leaving the game in the main slot running:
...
Switches the device to a [personal profile](../features/profiles.md) using its generated switch ID.
...
**`switch-id`** (required)
The switch ID generated by Core for the profile. Open the profile in the terminal UI to reveal or reset it. Selecting **Write Card** creates this command automatically.
...
Deactivates the active personal profile and returns the device to its [shared profile](../features/profiles.md#switch-profiles).
...
This command requires explicitly enabling the arguments in the [`allow_execute`](../core/config/zapscript.md#allow_execute) config option. It is always blocked from remote sources. Currently, [Zap Links](./zap-links.md) are the only remote source; scripts sent through the [Zaparoo App](../app/index.md) are checked against `allow_execute` normally.
:::
