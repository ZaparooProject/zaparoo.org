...
Linking a device does not turn on cloud backup, play history sync, Library sync, or remote control by itself. After linking, the terminal UI offers **Turn on everything** or **Let me choose**, and the **All online features** switch under **Settings > Online** turns them all on or off later. Either way, cloud backup is only turned on when the account has Warp.
...
To opt in, link the device if it is not already linked, then turn on **Library sync** under **Settings > Online** in the Core terminal UI, or **Sync library** on the [Zaparoo App](../app/index.md)'s Zaparoo Online settings page. The setting is [`library.sync`](../core/config/index.md#library-sync) in `config.toml`.
...
- Decks you make in Online appear on the device as [saved playlists](../features/playlists.md#deck), and decks made on the device, such as in the [Zaparoo App](../app/index.md#decks), appear in Online. Edits on either side are kept, and a deck edited in both places at once is merged. A deck locked in Online is read-only on the device.
...
On a device, a deck is a [saved playlist](../features/playlists.md#deck) that any token can open with `**playlist.play:deck://<id>`. Create and edit decks in Online or in the [Zaparoo App](../app/index.md#decks), and [Library sync](#library-sync) keeps the device and your account in step. A [Zap Link](../zapscript/zap-links.md) to a shared deck opens it as a playlist too, and the device keeps a read-only copy so the link works offline after that.
