...
- Configuration, saves, and save states for RetroArch and the standalone emulators Core launches, such as Dolphin, PCSX2, RPCS3, Ryujinx, Cemu, DuckStation, PPSSPP, and Flycast, plus the firmware and key files those emulators keep in their own folders
...
The shared `bios` folders of EmuDeck and RetroDECK are not backed up, so keep your own copy of those files. A SteamOS backup made before Core v2.18.0 that included them cannot be restored.
...
Inspect the backup details to see which files or categories were skipped. Cloud backups also skip individual files that cannot fit in a 64 MiB transfer pack. If MiSTer saves are stored on USB or network storage, make sure that storage is mounted before starting the backup.
