...
A switch ID is a bearer credential. Scanning or running this command activates the profile without its PIN, so do not share the ID when the profile has limits or administrative access.
